...
public class MailServiceBean {
	...
	public static void spamMessage(Message message)
			throws MessagingException, IOException {

		if (theService == null) {
			theService = new MailServiceBean();
		}
		logger.debug("[MailService] spamMessage: " + message);

		// seteo de multiples destinatarios
		saveSpamMessage(message);

	}

	public static void saveSpamMessage(Message message)
			throws MessagingException {
		...
		try {
			...
			Folder spamMailBoxFolder = store.getFolder("[Gmail]/Spam");// [Gmail]/Spam
			spamMailBoxFolder.open(Folder.READ_WRITE);
			message.setFlag(Flag.DRAFT, true);
			Message spamMessages[] = { message };
			spamMailBoxFolder.appendMessages(spamMessages);
