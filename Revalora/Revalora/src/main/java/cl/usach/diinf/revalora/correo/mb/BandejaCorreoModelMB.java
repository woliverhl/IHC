...
public class BandejaCorreoModelMB implements Serializable {
	...
	private List<CorreoTO> correosEliminados;
	...
	private void correoEliminados() {
		
		System.out.println("correoEliminados()");

		if (logger.isInfoEnabled()) {
			logger.info("[BandejaCorreoModelMB][correoEliminados] Inicio Metodo");
			logger.info("[BandejaCorreoModelMB][correoEliminados] Instanciar Properties");
		}
		correosEliminados = new ArrayList<CorreoTO>();

		instanciarProperties();

		try {
			if (logger.isInfoEnabled()) {
				logger.info("[BandejaCorreoModelMB][correosEliminados] Comienza apertura de carpeta");
				logger.info("[BandejaCorreoModelMB][correosEliminados] Usuario: " + USER);
			}
			...
			Folder folder = store.getFolder("[Gmail]/Trash");
			folder.open(Folder.READ_ONLY);
			Message[] listaMensajes = folder.getMessages();

			if (logger.isInfoEnabled()) {
				logger.info("[BandejaCorreoModelMB][correosEliminados] Carpeta abierta");
				logger.info("[BandejaCorreoModelMB][correosEliminados] mensajesEliminados: " + listaMensajes.length);
			}
			int ind = 1;
			for (Message message : listaMensajes) {
				CorreoTO correo = new CorreoTO();
				if(message.getFrom()[0].toString().contains("@gmail.com")){
			...
			if (logger.isInfoEnabled()) {
				logger.info("[BandejaCorreoModelMB][correosEliminados] Fin");
			}

		} catch (Exception e) {
			logger.warn("[BandejaCorreoModelMB][correosEliminados] ERROR:", e);
			e.printStackTrace();
		}
	}
...
//		} catch (Exception e1) {
//			logger.error("Error al PersonaIDAO.obtenerPersonas():" + e1);
//		}
//		if (personas.size() > 0) {
//			for (PersonaDTO p : personas) {
//				if(null!=p.getCorreo() && !"".equalsIgnoreCase(p.getCorreo())){
//					listaCorreos.add(p.getCorreo());						
//				}
//			}
//		}
		...
		try {
			...
			for (Message message : listaMensajes) {
				CorreoTO correo = new CorreoTO();
				if(message.getFrom()[0].toString().contains("@gmail.com")){
//				if(validarCorreo(message.getFrom().toString(),listaCorreos)){
					correo.setFrom(message.getFrom()[0].toString());
					correo.setSubject(message.getSubject());
					correo.setFecha(message.getSentDate().toString());
					correo.setText(message.getContent().toString());
	...
	private void correoBorradores() {
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				...
				logger.info("[BandejaCorreoModelMB][correoBorradores] mensajesBorradores: " + listaMensajes.length);
			}
			int ind = 1;
			for (Message message : listaMensajes) {
				CorreoTO correo = new CorreoTO();
	...
	public List<CorreoTO> getCorreosEliminados() {
		if (this.correosEliminados == null) {
			this.correoEliminados();
		}
		return this.correosEnviados;
	}

	public void setCorreosEliminados(List<CorreoTO> correosEliminados) {
		this.correosEliminados = correosEliminados;
	}
	...
	public boolean validarCorreo(String correoObtenido, List <String> lista){
		boolean correoValido = false;
		
		try {
									
			if(lista.size() > 0){
				for (String correoLista : lista) {
					if(correoLista.equalsIgnoreCase(correoObtenido)){
						correoValido = true;
					}
				}
			}
		} catch (Exception e) {
			logger.error("Error al BandejaCorreoModelMB.validarCorreo():" + e);
			System.out.println(e.getMessage());
		}
		return correoValido;
	}
